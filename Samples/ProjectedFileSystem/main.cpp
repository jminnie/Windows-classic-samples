...
int
__cdecl wmain(int argc, const WCHAR** argv)
{
    if (argc <= 1)
    {
        wprintf(L"Usage: \n");
    ...
    std::wstring virtRootPath = argv[1];
    ...
    PRJ_STARTVIRTUALIZING_OPTIONS startVirtOpts = {};
    startVirtOpts.NotificationMappings = notificationMappings;
    startVirtOpts.NotificationMappingsCount = 1;

    // Start the provider using the options we set up.
    RegfsProvider provider;
    auto hr = provider.Start(virtRootPath.c_str(), &startVirtOpts);
